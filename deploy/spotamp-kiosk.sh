#!/bin/bash
# SpotAmp — kiosk launcher (run by the systemd user service).
...
#
# Install: cp deploy/spotamp-kiosk.sh ~/.config/spotamp-kiosk.sh && chmod +x it.
export XDG_RUNTIME_DIR=/run/user/1000
export WAYLAND_DISPLAY=wayland-0
export SDL_VIDEODRIVER=wayland
cd /home/bkern/SpotAmp/pi || exit 1
for _ in $(seq 1 30); do
  [ -S "$XDG_RUNTIME_DIR/$WAYLAND_DISPLAY" ] && break
  sleep 1
done
wlr-randr --output DPI-1 --transform 180 2>/dev/null   # display + touch together
exec venv/bin/python -m spotamp
